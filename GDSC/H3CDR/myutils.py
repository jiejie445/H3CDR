...
def evaluate_metrics(true_data: torch.Tensor, predict_data: torch.Tensor, thresholds: torch.Tensor = None):
    """
    计算多个评价指标并返回结果字典。
    :param true_data: 真实标签，1D Tensor
    :param predict_data: 预测值，1D Tensor
    :param thresholds: 阈值，浮动
    :return: 各个指标的字典
    """
    assert torch.all(true_data.ge(0)) and torch.all(true_data.le(1)), "Out of range!"
    ...
    #AUPRC
    auprc = ap_score(true_data, predict_data)

    # F1 Score
    max_f1, threshold_f1 = f1_score_binary(true_data, predict_data)

    # Accuracy
    accuracy = accuracy_binary(true_data, predict_data, threshold_f1)

    # Precision
    precision = precision_binary(true_data, predict_data, threshold_f1)

    # Recall
    recall = recall_binary(true_data, predict_data, threshold_f1)

    # MCC
    mcc = mcc_binary(true_data, predict_data, threshold_f1)
...
def torch_corr_x_y(tensor1: torch.Tensor, tensor2: torch.Tensor):
    ...
    # std_x_y = torch.mm(torch.sqrt(lxx + 1e-10).view([-1, 1]), torch.sqrt(lyy + 1e-10).view([1, -1]))
...
def torch_euclidean_dist(tensor: torch.Tensor, dim=0):
    ...
    # print("欧氏距离矩阵形状为：",dist.shape)
    # print("欧氏距离矩阵中最大值",dist.max())
    # print("欧氏距离矩阵中最小值",dist.min())
    return dist

#############计算余弦距离矩阵
def torch_cosine_dist(tensor: torch.Tensor, dim=0):
    """
    :param tensor: a 2D torch tensor
    :param dim:
        0 : represent row
        1 : represent col
    :return: return cosine distance
    """
    if dim == 1:
        tensor = tensor.t()
#这种计算方式会导致对角线不为0
    # norm_tensor = torch.norm(tensor, p=2, dim=1, keepdim=True)
    # normalized_tensor = tensor / norm_tensor
    # cosine_similarity = torch.mm(normalized_tensor, normalized_tensor.t())
    # cosine_distance = 1 - cosine_similarity

    normalized_tensor = torch.nn.functional.normalize(tensor, p=2, dim=1)
    cosine_similarity = torch.mm(normalized_tensor, normalized_tensor.t())
    cosine_distance = 1 - cosine_similarity
...
def exp_similarity(tensor: torch.Tensor, sigma: torch.Tensor, normalize=True):
    ...
    # print("相似度矩阵形状为：",exp_dist.shape)
    # print("相似度矩阵中最大值",exp_dist.max())
    # print("相似度矩阵中最小值",exp_dist.min())
    ##方法二使用1/(1+距离)返回相似度矩阵
    # exp_dist = 1 / (1 + tensor_dist)
    #
    # ##方法三使用归一化相似度返回相似度矩阵
    # min_dist = torch.min(tensor_dist)
    # max_dist = torch.max(tensor_dist)
    # exp_dist = 1 - (tensor_dist - min_dist) / (max_dist - min_dist)

    # ##方法四使用拉普拉斯核矩阵返回相似度矩阵
    # exp_dist = torch.exp(-tensor_dist/sigma)
...
#     :param sigma: scale parameter
#     :param normalize: normalize or not
#     :param use_cosine: use cosine similarity instead of euclidean distance
#     :return: exponential similarity
#     """
#     if normalize:
#         tensor = torch_z_normalized(tensor, dim=1)
#     tensor_dist = torch_cosine_dist(tensor, dim=0)
#
#     ###方法一使用高斯相似度返回相似度矩阵
#     exp_dist = torch.exp(-tensor_dist / (2 * torch.pow(sigma, 2)))
#
...
def scale_sigmoid_activation_function(tensor: torch.Tensor, alpha: int or float):
    ...
    return output
def scale_sigmoid(tensor: torch.Tensor, alpha: 8):
    alpha = torch.tensor(alpha, dtype=torch.float32, device=tensor.device)
