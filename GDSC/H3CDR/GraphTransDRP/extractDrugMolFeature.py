...
from torch_geometric.nn import GATConv, GCNConv, global_mean_pool, JumpingKnowledge, global_max_pool
...
class ImprovedDrugGraphGNN(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=4):
        ...
        self.fc = torch.nn.Linear(hidden_dim * num_layers, output_dim)
...
# 使用transformer进行药物数据的特征提取
class DrugDataTransformer(torch.nn.Module):
    def __init__(self, num_features_xd=75, output_dim=384):
        super(DrugDataTransformer, self).__init__()
...
def process_and_extract_features(dataset_path, input_dim=75, hidden_dim=128, output_dim=228, batch_size=32):
    # Load dataset
...
if __name__ == "__main__":
    ...
    print("经过特征提取后的药物特征矩阵为：",features.shape)
