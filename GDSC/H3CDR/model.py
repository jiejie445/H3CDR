...
class ConstructAdjMatrix(nn.Module, ABC):
    def __init__(self, original_adj_mat, nullMask, device="gpu"):
    ...
    def forward(self):
        ...
        # U_adj_matrix = torch.mm(adj_matrix, adj_matrix)                  #GUN网络中的邻接矩阵，做了一次平方-----不建议简单做平方。
        d = torch.diag(torch.pow(torch.sum(adj_matrix, dim=1), -1/2))
        identity = torch.eye(d.shape[0], dtype=torch.float, device=self.device)
        adj_matrix_hat = torch.mm(d, torch.mm(adj_matrix, d))
        adj_matrix_hat = torch.add(identity, adj_matrix_hat)

        # GUN网络中的邻接矩阵-对A做复杂三次幂运算
...
#计算同构图需要的邻接矩阵，得到的应该是962*962 或者是228*228的矩阵
...
class concatCellAndDrug(nn.Module, ABC):
    ...
    def forward(self):
        ...
        adj_matrix = torch.cat((cell_drug, drug_cell), dim=0)
        identity = torch.eye(1190, dtype=torch.float, device=self.device)
...
class OnlySimilarity(nn.Module, ABC):
    ...
    def forward(self):
        ...
        drug_similarity = full_kernel(self.drug_jac_similarity)      # 原方法-使用子结构指纹计算相似性
...
# class OnlyConcatMatrix(nn.Module, ABC):
#     def __init__(self, cellAdjMatrix, drugAdjMatrix, device="gpu"):
#         super(OnlyConcatMatrix, self).__init__()
#         self.cell_adj_mat = cellAdjMatrix.to(device)
#         self.drug_adj_mat = drugAdjMatrix.to(device)
#         self.device = device
#
#     def forward(self):
#         n_cell = self.cell_adj_mat.shape[0]
#         n_drug = self.drug_adj_mat.shape[1]
#         cell_identity = torch.eye(n_cell, dtype=torch.float, device=self.device)
#         drug_identity = torch.eye(n_drug, dtype=torch.float, device=self.device)
#         cell_drug = torch.cat((cell_identity, self.adj_mat), dim=1)
#         drug_cell = torch.cat((torch.t(self.adj_mat), drug_identity), dim=1)
...
class FusionFeature(nn.Module, ABC):
    ...
    def forward(self):
        ...
        # drug_similarity = self.drug_graph_similarity
        # drug_similarity = (full_kernel(self.drug_jac_similarity) + full_kernel(self.drug_graph_similarity))/2
        cell_similarity = self.fusion_cell_feature()
        zeros1 = torch.zeros(cell_similarity.shape[0], drug_similarity.shape[1], dtype=torch.float32, device=self.device)
        zeros2 = torch.zeros(drug_similarity.shape[0], cell_similarity.shape[1], dtype=torch.float32, device=self.device)
...
class GraphUnet(nn.Module):
    ...
    def forward(self, g, h):
        ...
        # 解码阶段
        for i in range(self.l_n):
            up_idx = self.l_n - i - 1          #i=0 , up_idx=1, i=1 , up_idx = 0,
...
class GDecoder(nn.Module, ABC):
    def __init__(self, n_cell, n_drug, n_hid1, n_hid2, alpha, device):
        ...
        self.n_drug = n_drug
        self.alpha = alpha
        # self.lm_cell = nn.Linear(n_hid1, n_hid2, bias=False)  # 线性变换：变换矩阵大小为：384*72
        # self.lm_drug = nn.Linear(n_hid1, n_hid2, bias=False)
        self.lm_cell = nn.Linear(962, 128, bias=False)
        self.lm_drug = nn.Linear(228, 128, bias=False)
    ...
    def forward(self, GUN_output,  GCN_output, cellGCN, drugGCN):
        z_cell = GUN_output[:962, :962]
        z_drug = GUN_output[962:, 962:]

        GCN_cell = GCN_output[:962, :962]
        GCN_drug = GCN_output[962:, 962:]
        # z_cell, z_drug = torch.split(encode_output, [self.n_cell, self.n_drug], dim=0)
        z_cell = torch.add(z_cell, cellGCN)
        z_drug = torch.add(z_drug, drugGCN)
        #额外再加上GCN的特征
        z_cell = torch.add(z_cell, GCN_cell)
        z_drug = torch.add(z_drug, GCN_drug)
...
#         return output                                                                                                 ##------------------------------------GCN的Model

####################第一版GAT的model
    # def __init__(self, adj_mat, gene, cna, mutation, sigma, k, iterates, feature_drug, n_hid1, n_hid2, alpha, n_heads,
    #              device="gpu"):
    #     super(GModel, self).__init__()
    #     construct_adj_matrix = ConstructAdjMatrix(adj_mat, device=device)
    #     fusioner = FusionFeature(gene, cna, mutation, sigma=sigma, k=k, iterates=iterates, feature_drug=feature_drug,
    ...
    #     adj_matrix_hat = construct_adj_matrix()
    #     feature = fusioner()
    #     self.encoder = GEncoder(adj_matrix_hat, feature, n_hid1, n_heads=n_heads)
    #     self.decoder = GDecoder(adj_mat.shape[0], adj_mat.shape[1], n_hid1=n_hid1, n_hid2=n_hid2, alpha=alpha)
    #
    # def forward(self):
    #     encode_output = self.encoder()
...
###########################第二版GAT的model
    ...
    #     adj_matrix_hat = construct_adj_matrix()
    #     feature = fusioner()
    #     self.encoder = GEncoder(adj_matrix_hat, feature, n_hid1, n_heads=n_heads, n_layers=n_layers, dropout=dropout)
    #     self.decoder = GDecoder(adj_mat.shape[0], adj_mat.shape[1], n_hid1=n_hid1, n_hid2=n_hid2, alpha=alpha)
    #
    # def forward(self):
    #     encode_output = self.encoder()
...
#         adj_matrix_hat = construct_adj_matrix()
#         feature = fusioner()
#         self.encoder = GATEncoder(adj_matrix_hat, feature, n_hid1, num_heads=num_heads)
#         self.decoder = GDecoder(adj_mat.shape[0], adj_mat.shape[1], n_hid1=n_hid1, n_hid2=n_hid2, alpha=alpha)
#
#     def forward(self):
#         encode_output = self.encoder()
...
class GModel(nn.Module, ABC):
    def __init__(self, adj_mat, gene, cna, mutation, null_mask, sigma, k, iterates, feature_drug, graph_feature_drug,  n_hid1,
        ...
        self.cellFeature, self.drugFeature, self.drug_graph_feature = self.similarFeature()       #!!   得到细胞系和药物各自的相似性矩阵，是未拼接在一起的相似性特征
        self.GCNCellFeature = GCNLayer(962, 962)     #!!     定义细胞系的同构GCNLayer结构
        self.GCNDrugFeature = GCNLayer(228, 228)     #!!     定义药物的同构GCNLayer结构
        ...
        #原来的异构GCN结构
        self.gcnEncoder = GEncoder(self.adj_matrix_hat,self.feature,1190)
        # 解码器拆分特征矩阵
    ...
    def forward(self):
        ...
        gcnDrugFeature = self.GCNDrugFeature(self.drug_adj_matrix, self.drugFeature)     #!!
        # gcnDrugFeature = self.GCNDrugFeature(self.drug_adj_matrix, self.drug_graph_feature)     #!!数据层面的消融实验-去掉药物的子结构指纹
        ...
        # output = self.decoder(h, gcnCellFeature, gcnDrugFeature)    #消融实验去掉GCN分支
